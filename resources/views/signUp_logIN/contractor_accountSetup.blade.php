...
<body class="min-h-screen">
	<div class="setup-container">
		...
		<div class="setup-form-wrapper">
			<form class="setup-form" method="POST" action="{{ url('/contractor/account-setup') }}" enctype="multipart/form-data">
				@csrf
				...
				<div class="field-stack">
					...
					<div class="field-block">
						<label class="field">
							...
							<input type="tel" name="company_phone" placeholder="Company Phone (e.g., 09171234567)" maxlength="11" required>
						</label>
						<p style="font-size: 12px; color: var(--muted-color); margin: 4px 0 0;">11 digits starting with 09</p>
					</div>

					<div class="field-block">
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-calendar"></i></span>
							<input type="date" name="founded_date" id="founded_date" placeholder="Founding Date" required>
						</label>
					</div>
					...
					<div class="field-block">
						<label class="field">
							...
							<input type="text" id="years_of_experience" placeholder="Years of Experience" readonly style="background-color: #f5f5f5; cursor: not-allowed;">
						</label>
					</div>

					<div class="field-block">
						<input type="hidden" name="contractor_type_id" id="contractorTypeValue" value="{{ old('contractor_type_id') }}" required>
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-briefcase"></i></span>
					...
					<div class="field-block" id="contractor_type_other_wrapper" style="display: none;">
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-briefcase"></i></span>
							<input type="text" name="contractor_type_other_text" placeholder="Specify Other Contractor Type">
						</label>
					</div>
					...
					<div class="field-block">
						<label class="field">
							...
							<input type="text" name="business_address_street" placeholder="Street Address / Building / House No." required>
						</label>
					</div>

					<div class="field-block">
						<input type="hidden" name="business_address_province" id="provinceValue" value="{{ old('business_address_province') }}" required>
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-marker"></i></span>
					...
					<div class="field-block">
						<input type="hidden" name="business_address_city" id="cityValue" value="{{ old('business_address_city') }}" required>
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-marker"></i></span>
					...
					<div class="field-block">
						<input type="hidden" name="business_address_barangay" id="barangayValue" value="{{ old('business_address_barangay') }}" required>
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-marker"></i></span>
					...
					<div class="field-block">
						<label class="field">
							...
							<input type="url" name="company_website" id="company_website" placeholder="https://example.com">
						</label>
						<p style="font-size: 12px; color: var(--muted-color); margin: 4px 0 0;">Enter full URL (e.g., https://yoursite.com)</p>
					</div>
					<div class="field-block">
						<label class="field">
							<span class="field-icon" aria-hidden="true"><i class="fi fi-rr-globe"></i></span>
							<input type="text" name="company_social_media" placeholder="Social Media (Facebook, Instagram, etc.)">
						</label>
					</div>
				</div>
			</div>
			...
			<div class="form-step" id="step-2">
				<div class="field-stack">
					...
					<p style="font-size: 14px; font-weight: 600; color: var(--text-color); margin: 0 0 16px 0;">Personal Information</p>
					...
					<p style="font-size: 14px; font-weight: 600; color: var(--text-color); margin: 24px 0 16px 0;">Account Credentials</p>
					...
					<div class="field-block">
						<label class="field">
							...
							<input type="password" name="password" id="passwordInput" placeholder="Password (min 8 characters)" required>
						</label>
						...
						<p style="font-size: 12px; color: var(--muted-color); margin: 4px 0 0;">Minimum 8 characters</p>
					</div>
					...
					<div class="field-block">
						...
						<p id="passwordMismatchError" style="font-size: 12px; color: #d32f2f; margin: 4px 0 0; display: none;">Passwords do not match</p>
					</div>
				</div>
			</div>
			...
			<div class="form-step" id="step-3">
				<div class="field-stack">
					...
					<p style="font-size: 18px; font-weight: bold; color: var(--text-color); margin: 0 0 20px 0;">PICAB Certification</p>
					...
					<div class="field-block">
						...
						<label class="field">
							...
							<input type="hidden" name="pcab_category" id="pcabCategoryValue" value="{{ old('pcab_category') }}" required>
							<button type="button" class="select-button" onclick="openModal('pcabCategoryModal')">
								<span id="pcabCategoryDisplay">{{ old('pcab_category') ?: 'Select PCAB Category' }}</span>
								<i class="fi fi-rr-angle-small-down"></i>
							</button>
						</label>
					</div>
					...
					<p style="font-size: 18px; font-weight: bold; color: var(--text-color); margin: 32px 0 20px 0;">Business Permit</p>
					...
					<p style="font-size: 18px; font-weight: bold; color: var(--text-color); margin: 32px 0 20px 0;">Business Registration</p>
					...
					<div class="field-block">
						...
						<input type="file" name="dti_sec_registration" class="upload-input" id="dtiSecInput" accept="image/jpeg,image/jpg,image/png,application/pdf" required>
						</div>
					</div>
				</div>

				<p style="font-size: 12px; color: var(--muted-color); margin-top: 12px; text-align: center;">
					By submitting you agree to our Terms & Privacy. All files should be valid and not expired. We'll verify these documents before approving your profile.
				</p>
			</div>
