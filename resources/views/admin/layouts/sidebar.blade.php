    ...
    <div class="nav-group">
      ...
      <div class="nav-submenu">
        <a href="{{ route('admin.userManagement.propertyOwner') }}"
           class="submenu-link {{ request()->routeIs('admin.userManagement.propertyOwner*') ? 'active' : '' }}">Property Owner</a>
        ...
        <a href="{{ route('admin.userManagement.verificationRequest') }}"
           class="submenu-link {{ request()->routeIs('admin.userManagement.verificationRequest') ? 'active' : '' }}">Verification Request</a>
        <a href="{{ route('admin.userManagement.suspendedAccounts') }}"
           class="submenu-link {{ request()->routeIs('admin.userManagement.suspendedAccounts') ? 'active' : '' }}">Suspended Accounts</a>
      </div>
    </div>
    ...
    <div class="nav-group">
      ...
      <div class="nav-submenu">
        ...
        <a href="{{ route('admin.globalManagement.aiManagement') }}"
           class="submenu-link {{ request()->routeIs('admin.globalManagement.aiManagement') ? 'active' : '' }}">AI Management</a>
        <a href="{{ route('admin.globalManagement.postingManagement') }}"
           class="submenu-link {{ request()->routeIs('admin.globalManagement.postingManagement') ? 'active' : '' }}">Posting Management</a>
      </div>
    </div>
    ...
    <div class="nav-group">
      ...
      <div class="nav-submenu">
        <a href="{{ route('admin.projectManagement.listOfProjects') }}"
           class="submenu-link {{ request()->routeIs('admin.projectManagement.listOfProjects') ? 'active' : '' }}">List of Projects</a>
...
<div id="logoutModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-[100] transition-opacity duration-200">
  <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 overflow-hidden transform transition-all duration-200 scale-95" id="logoutModalContent">
    <div class="px-6 py-5 bg-gradient-to-r from-red-600 to-rose-600">
      <div class="flex items-center justify-between">
        ...
        <button id="logoutModalClose" class="text-white/80 hover:text-white transition text-2xl leading-none cursor-pointer">&times;</button>
      </div>
    </div>
    <div class="p-6 space-y-5">
      ...
      <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-200">
        <button id="logoutModalCancel" class="px-6 py-2.5 rounded-xl border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition cursor-pointer">Cancel</button>
        <button id="logoutModalConfirm" class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-semibold shadow-md hover:shadow-lg transition cursor-pointer">Confirm Logout</button>
      </div>
    </div>
  </div>
</div>
