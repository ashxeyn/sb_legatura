document.addEventListener('DOMContentLoaded', () => {
	...
	const showStep = (step) => {
		...
		const subtitles = [
			'Let\'s get your profile started with some basic information.',
		...
		if (titleEl) titleEl.textContent = `Account Setup - ${step}`;
	...
	const setupUploadAreas = () => {
		...
		const clearanceImageInput = document.getElementById('clearanceImageInput');

		[uploadArea1, uploadArea2].forEach((area, index) => {
			const input = index === 0 ? idImageInput : clearanceImageInput;
