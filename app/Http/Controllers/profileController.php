...
class profileController extends Controller
{
    ...
    public function apiGetProfile(Request $request)
    {
        ...
        if ($activeRole === 'contractor') {
            // Resolve contractor in a schema-agnostic way (avoid contractors.user_id queries)
            $contractor = $this->getContractorForUser($userId);
            ...
            if ($contractor) {
                // Some deployments keep the contractor's linked user on `contractors.user_id`,
                // others use `contractors.owner_id` which links to `property_owners.owner_id`.
                // Resolve the actual user_id for the contractor safely.
                $contractorUserId = $contractor->user_id ?? null;
                if (empty($contractorUserId) && isset($contractor->owner_id)) {
                    $po = DB::table('property_owners')->where('owner_id', $contractor->owner_id)->first();
                    $contractorUserId = $po->user_id ?? null;
                }
                ...
                if ($contractorUserId) {
                    ...
                    // Could not resolve a linked user for this contractor -> no reviews
