	...
	private function checkOwnerAccess(Request $request)
	{
		...
		if ($user->user_type === 'both') {
			...
			if ($currentRole !== 'owner') {
				if ($request->expectsJson()) {
					return response()->json(['success' => false, 'message' => 'Please switch to the owner dashboard to upload payments.'], 403);
				}
				return redirect('/dashboard')->with('error', 'Please switch to the owner dashboard to upload payments.');
			}
		}
	...
	public function uploadPayment(paymentUploadRequest $request)
	{
		...
		// Get primary contractor user (owner role preferred, or first active user, then any user)
			...
			->select(
				...
				'pr.owner_id',
				'c.contractor_id',
				'cpo.owner_id as contractor_user_id',
		...
		if (!$project->contractor_id) {
			return response()->json(['success' => false, 'message' => 'Contractor profile not found for this project. Please contact support.'], 403);
		}
			...
			'owner_id' => $ownerId,
			'contractor_id' => (int) $project->contractor_id,
