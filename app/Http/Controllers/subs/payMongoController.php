...
class payMongoController extends Controller
{
    ...
    public function createSubscriptionCheckout(Request $request)
    {
        ...
        $planKey = $request->input('plan_tier'); // 'gold', 'silver', 'bronze'
        Log::info('createSubscriptionCheckout: Plan Key ' . $planKey);

        $plan = subscriptionPlan::where('plan_key', $planKey)->where('is_active', true)->first();
        ...
        if ($checkoutAmount < 2000) {
            ...
            return response()->json(['success' => false, 'message' => 'Plan amount must be at least 20 PHP'], 400);
        }
        if ($checkoutAmount > 99999999 / 100 * 100) { // Keep safety margin for 999,999 pesos
            Log::error('createSubscriptionCheckout: Plan amount exceeds PayMongo maximum limit. Plan: ' . $planKey . ' Amount: ' . $checkoutAmount);
            return response()->json(['success' => false, 'message' => 'Plan amount exceeds maximum payment limit'], 400);
        }
        ...
        try {
            ...
            $successUrl = $returnUrl ? ($returnUrl . (strpos($returnUrl, '?') !== false ? '&' : '?') . 'status=success') : route('payment.callback', ['type' => 'subscription', 'plan' => $planKey]);
            ...
            if ($response->successful()) {
                ...
                $expirationDate = $this->calculateExpirationDate($plan->billing_cycle, $plan->duration_days);
    ...
    public function createBoostCheckout(Request $request)
    {
        ...
        $planKey = $request->input('plan_key', 'boost'); // Allow dynamic plan key, default to boost
        $boostPlan = DB::table('subscription_plans')->where('plan_key', $planKey)->first();

        if (!$boostPlan) {
            Log::error('createBoostCheckout: Plan not found: ' . $planKey);
            return response()->json(['success' => false, 'message' => 'Plan not found'], 404);
        }

        $secretKey = env('PAYMONGO_TEST_SECRET_KEY', env('PAYMONGO_SECRET_KEY'));
        $checkoutAmount = $boostPlan->amount; // Amount is already in centavos in the DB

        if ($checkoutAmount < 2000) {
            Log::error('createBoostCheckout: Plan amount is less than PayMongo minimum 20 PHP (2000 centavos). Plan: ' . $planKey . ' Amount: ' . $checkoutAmount);
            return response()->json(['success' => false, 'message' => 'Boost amount must be at least 20 PHP'], 400);
        }
        if ($checkoutAmount > 99999999 / 100 * 100) { // Keep safety margin for 999,999 pesos
            Log::error('createBoostCheckout: Plan amount exceeds PayMongo maximum limit. Plan: ' . $planKey . ' Amount: ' . $checkoutAmount);
            return response()->json(['success' => false, 'message' => 'Boost amount exceeds maximum payment limit'], 400);
        }

        $checkoutName = $boostPlan->name;
        $checkoutDescription = $boostPlan->billing_cycle === 'one-time' && $boostPlan->duration_days > 0
            ? $boostPlan->name . ' - ' . $boostPlan->duration_days . ' Days'
            : $boostPlan->name . ' Subscription';
            ...
            if ($response->successful()) {
                ...
                try {
                    ...
                    DB::table('platform_payments')->insert([
                        ...
                        'project_id' => $projectId,
                        'subscriptionPlanId' => $boostPlan->id,
                        'amount' => $boostPlan->amount / 100, // Convert centavos to pesos for DB storage
                        'transaction_number' => $checkoutSessionId,
                        'is_approved' => 0,
                        'transaction_date' => now(),
                        'expiration_date' => $this->calculateExpirationDate($boostPlan->billing_cycle, $boostPlan->duration_days),
                        'payment_type' => 'PayMongo'
                    ]);
                } catch (\Exception $dbEx) {
    ...
    public function cancelSubscription(Request $request)
    {
        ...
        try {
            ...
            $affected = DB::table('platform_payments')
                ...
                ->whereNotNull('subscriptionPlanId') // Non-null plan ID indicates a plan (subscription or boost)
                ->where('is_approved', 1)
                ->where(function ($q) {
    ...
    public function verifyBoostPayment(Request $request)
    {
        ...
        if (!$transactionNumber) {
            ...
            $boostPlan = subscriptionPlan::where('plan_key', 'boost')->first();

            $payment = DB::table('platform_payments')
                ->where('project_id', $projectId)
                ->where('subscriptionPlanId', $boostPlan->id ?? null)
                ->where('is_approved', 0)
                ->orderByDesc('platform_payment_id')
                ->first();
    ...
    /**
     * Helper to calculate expiration date based on billing cycle and duration.
     */
    private function calculateExpirationDate($billingCycle, $durationDays = null)
    {
        $expirationDate = now()->addMonth(); // default fallback

        if (in_array($billingCycle, ['yearly', 'annual'])) {
            $expirationDate = now()->addYear();
        } elseif ($billingCycle === 'quarterly') {
            $expirationDate = now()->addMonths(3);
        } elseif ($billingCycle === 'one-time') {
            $expirationDate = $durationDays > 0 ? now()->addDays($durationDays) : now()->addMonth();
