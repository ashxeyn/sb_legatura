...
class reviewsClass
{
    ...
    public function fetchReviews($filters = [])
    {
        $query = DB::table('reviews as r')
            ...
            ->leftJoin('contractors as c_rev', 'u_rev.user_id', '=', 'c_rev.user_id')
            // Join for reviewee names
            ->leftJoin('property_owners as po_ree', 'u_ree.user_id', '=', 'po_ree.user_id')
            ->leftJoin('contractors as c_ree', 'u_ree.user_id', '=', 'c_ree.user_id')
            ->select(
                'r.review_id',
                'r.rating',
                ...
                'r.created_at',
                'p.project_title',
                'u_rev.profile_pic as reviewer_pic',
                'u_rev.user_type as reviewer_type',
                DB::raw("CASE 
                    WHEN u_rev.user_type = 'contractor' THEN c_rev.company_name 
                    WHEN u_rev.user_type = 'property_owner' THEN CONCAT(po_rev.first_name, ' ', po_rev.last_name)
                    ELSE u_rev.username 
                END as reviewer_name"),
                'u_ree.profile_pic as reviewed_pic',
                'u_ree.user_type as reviewed_type',
                DB::raw("CASE 
                    WHEN u_ree.user_type = 'contractor' THEN c_ree.company_name 
                    WHEN u_ree.user_type = 'property_owner' THEN CONCAT(po_ree.first_name, ' ', po_ree.last_name)
                    ELSE u_ree.username 
